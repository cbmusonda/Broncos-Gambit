...
static int quiescence(const Pos *p, int alpha, int beta, int qdepth) {
    ...
    int in_chk = in_check(p, p->white_to_move);
    ...
    /* When in check search ALL legal moves; otherwise captures+promotions only */
    Move all[320]; int an = pseudo_legal_moves(p, all);
    Move ms[320];  int n  = 0;
    if (in_chk) {
        for (int i = 0; i < an; i++) ms[n++] = all[i];
    } else {
        for (int i = 0; i < an; i++)
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (in_check(&np, !np.white_to_move)) continue;
...
static Move find_best_move(const Pos *p, const u64 *gh, int ghl, int max_depth, int time_limit_ms) {
    Move ms[320]; int n = legal_moves(p, ms);
    ...
    for (int depth = 1; depth <= max_depth; depth++) {
        ...
        best = cur_best;
        if(time_limit_ms > 0) {
            int elapsed_ms = (int)((clock() - t_start) * 1000 / CLOCKS_PER_SEC);
            if(elapsed_ms >= time_limit_ms * 8 / 10) break;
        }
    }
    return best;
}
...
int main(void) {
    ...
    while (fgets(line, sizeof(line), stdin)) {
        ...
        if (strcmp(line,"uci")==0) {
            ...
            int time_limit_ms = 0;
            char *mt = strstr(line, "movetime");
            if (mt) time_limit_ms = atoi(mt + 9);
            Move ms[320]; int n=legal_moves(&pos,ms);
            if (n<=0) { printf("bestmove 0000\n"); fflush(stdout); }
            else print_bestmove(find_best_move(&pos, gh, ghl, MAX_DEPTH, time_limit_ms));
